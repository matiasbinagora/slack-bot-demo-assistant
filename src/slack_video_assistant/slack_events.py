...
class SlackEventHandler:
    ...
    def handle_message(self, *, body: Mapping[str, Any], ack: AckCallable, client: Any) -> None:
        ...
        context = ThreadContext(
            ...
            thread_ts=str(event.get("thread_ts", "") or event.get("ts", "")).strip() or None,
        )
...
def _resolve_thread_context(body: Mapping[str, Any], file_info: Mapping[str, Any]) -> ThreadContext:
    ...
    share_context = _extract_share_context(shares, preferred_channel=explicit_channel)
    if share_context is None:
        return ThreadContext(team_id=team_id, channel_id=explicit_channel, thread_ts=explicit_thread)

    channel_id, thread_ts = share_context
    return ThreadContext(
        team_id=team_id,
        channel_id=explicit_channel or channel_id,
        thread_ts=explicit_thread or thread_ts,
    )


def _extract_share_context(
    ...
    for channel_map in channel_maps:
        for channel_id, entries in channel_map.items():
            ...
            for entry in entries:
                ...
                candidate = (str(channel_id), thread_ts)
                if preferred_channel and channel_id == preferred_channel:
                    return candidate
                if fallback is None:
                    fallback = candidate
    return fallback
